...
nba_player_box_games <- function(y) {
  ...
  if (length(season_player_box_list) > 0) {
    cli::cli_progress_step(
      msg = "Compiling {y} ESPN NBA Player Boxscores ({length(season_player_box_list)} games)",
    ...
    cli::cli_progress_step(
      msg = "Updating {y} ESPN NBA Player Boxscores GitHub Release",
  ...
  if (nrow(espn_df) > 1) {
    ...
    ifelse(
      !dir.exists(file.path("nba/player_box")),
      dir.create(file.path("nba/player_box")),
      FALSE
    )
    ...
    ifelse(
      !dir.exists(file.path("nba/player_box/rds")),
      dir.create(file.path("nba/player_box/rds")),
      FALSE
    )
    saveRDS(espn_df, glue::glue("nba/player_box/rds/player_box_{y}.rds"))

    ifelse(
      !dir.exists(file.path("nba/player_box/parquet")),
      dir.create(file.path("nba/player_box/parquet")),
      FALSE
    )
    arrow::write_parquet(
      espn_df,
      glue::glue("nba/player_box/parquet/player_box_{y}.parquet")
    )
  ...
  if (nrow(espn_df) > 0) {
    ...
    cli::cli_alert_info(
      "{length(season_player_box_list)} ESPN NBA Player Boxscores to be compiled for {y}, skipping Player Boxscores compilation"
    )
...
purrr::insistently(
  ...
  data_frame = final_sched,
  file_name = glue::glue("nba_schedule_master"),
  sportsdataverse_type = "schedule data",
  release_tag = "espn_nba_schedules",
  pkg_function = "hoopR::load_nba_schedule()",
  file_types = c("rds", "csv", "parquet"),
...
purrr::insistently(
  ...
  file_name = glue::glue("nba_games_in_data_repo"),
  sportsdataverse_type = "schedule data",
  release_tag = "espn_nba_schedules",
  pkg_function = "hoopR::load_nba_schedule()",
  file_types = c("rds", "csv", "parquet"),
