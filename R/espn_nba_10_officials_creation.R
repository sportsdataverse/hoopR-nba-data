...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "nba/officials/nba_officials_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("nba/officials")),
    dir.create(file.path("nba/officials"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
    season = as.integer(season),
...
build_season_officials <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN NBA officials ({length(game_ids)} games)",
    msg_done = "Compiled {y} ESPN NBA officials!"
  )
  ...
  rosters_df <- rosters_df %>%
    ...
    hoopR:::make_hoopR_data(
      "ESPN NBA Officials from hoopR data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("nba/officials/rds")),
    dir.create(file.path("nba/officials/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("nba/officials/parquet")),
    dir.create(file.path("nba/officials/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(
    rosters_df,
    glue::glue("nba/officials/rds/officials_{y}.rds")
  )
  arrow::write_parquet(
    rosters_df,
    glue::glue("nba/officials/parquet/officials_{y}.parquet")
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN NBA Officials GitHub Release",
    msg_done = "Updated {y} ESPN NBA Officials GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_nba_officials",
...
tryCatch({
  ...
  upload_nba_manifest(
    ...
    pkg_function         = "hoopR::load_nba_officials_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
